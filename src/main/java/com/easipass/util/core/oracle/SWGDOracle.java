package com.easipass.util.core.oracle;

import com.easipass.util.core.C3p0Properties;
 ...
 * @author ZJ
 * */
public final class SWGDOracle extends Database {
    ...
    static {
        C3p0Properties.getC3P0Properties().setData(COMBO_POOLED_DATA_SOURCE);

        final SWGDDatabaseProperties SWGD_PROPERTIES = SWGDDatabaseProperties.getInstance();

        try {
            COMBO_POOLED_DATA_SOURCE.setDriverClass(DRIVER_CLASS);
        } catch (PropertyVetoException e) {
            throw new ErrorException(e.getMessage());
        }
        COMBO_POOLED_DATA_SOURCE.setJdbcUrl("jdbc:oracle:thin:" + SWGD_PROPERTIES.getUrl() + ":" + SWGD_PROPERTIES.getPort() + ":" + SWGD_PROPERTIES.getSid());
        COMBO_POOLED_DATA_SOURCE.setUser(SWGD_PROPERTIES.getUsername());
        COMBO_POOLED_DATA_SOURCE.setPassword(SWGD_PROPERTIES.getPassword());
    }
